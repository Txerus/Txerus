...
private final class NativeAirPlayPlayerPlugin: NSObject, FlutterPlugin, AVPlayerViewControllerDelegate, UIAdaptivePresentationControllerDelegate {
  ...
  func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "open":
      ...
      presentPlayer(url: url, title: title, headers: headers, startMs: startMs, result: result)
  ...
  private func presentPlayer(
    ...
    presenter.present(controller, animated: true) { [weak self] in
      ...
      if startMs > 0 {
        let target = CMTime(value: startMs, timescale: 1000)
  ...
  private func finishNativePlayback() {
    ...
    let positionMs = position.isFinite ? Int64(max(0, position) * 1000) : 0
    let durationMs = duration.isFinite ? Int64(max(0, duration) * 1000) : 0
    player?.pause()
    pendingResult?([
